...
- [ ] **`ASSINATURA_REVALIDACAO` com o valor EXATAMENTE `on`** (sem espaço, sem
      quebra de linha). ⚠️ Visto AO VIVO no preview: uma env que existia mas com
      valor "quase on" deixou a revalidação **desligada em silêncio** —
      `revalidacaoLigada()` faz `=== 'on'`, então `"on\n"` ou `"On "` valem OFF.
      Com a revalidação off, **o corte de acesso não acontece** (o JWT de 30 dias
      nunca é reavaliado). Criar via CLI com `printf 'on' | vercel env add …`
      (nunca `echo`, que anexa `\n`). O mesmo vale para `ASSINATURA_NOVO_CADASTRO`.
