...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [a] = await prisma.$queryRawUnsafe<{ url: string }[]>(
    `SELECT "url" FROM "EstudioAsset" WHERE "id"=$1 AND "workspaceId"=$2 AND "tipo" IN ('molde','mockup','gerado','imagem')`, id, c.workspaceId)
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE "EstudioAsset" SET "url"=$3, "mime"=COALESCE($4,"mime"), "tamanhoBytes"=$5,
       "meta" = "meta" || jsonb_build_object('versao', COALESCE(("meta"->>'versao')::int, 1) + 1, 'trocadoEm', now()::text,
                                             'largura', $6::int, 'altura', $7::int)
     WHERE "id"=$1 AND "workspaceId"=$2`,
    id, c.workspaceId, b.url, typeof b.mime === 'string' ? b.mime : null, Math.max(0, Number(b.tamanhoBytes) || 0),
    Math.round(Number(b.largura) || 0), Math.round(Number(b.altura) || 0))
  if (storageConfigurado() && urlDoBlob(a.url) && a.url !== b.url) { try { await del(a.url) } catch { /* já sumiu */ } }
  ...
  return NextResponse.json({ ok: true, url: b.url, usadoEm: u?.n ?? 0 })
}
