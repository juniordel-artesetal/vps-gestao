...
export async function GET(req: NextRequest) {
  ...
  const rows = await prisma.$queryRawUnsafe(
    `SELECT "id","tipo","nome","url","mime","tamanhoBytes","pasta","tags","pedidoId","meta","createdAt"
     FROM "EstudioAsset"
     WHERE "workspaceId"=$1
...
export async function POST(req: NextRequest) {
  ...
  await prisma.$executeRawUnsafe(
    `INSERT INTO "EstudioAsset" ("id","workspaceId","tipo","nome","url","mime","tamanhoBytes","pasta","tags","pedidoId","meta")
    ...
    b.pedidoId || null, JSON.stringify(b.meta && typeof b.meta === 'object' ? b.meta : {}))
