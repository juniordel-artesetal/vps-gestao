...
import { notificarIndicacao } from '@/lib/telegramNotify'
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Notifica Telegram ────────────────────────────────────────────
    await notificarIndicacao({
      workspaceNome: session.user.workspaceNome || 'Ateliê',
      nomeIndicada: nomeIndicada.trim(),
      contatoIndicada: contatoIndicada.trim(),
    }).catch(e => console.error('[TELEGRAM indicacao]', e))
