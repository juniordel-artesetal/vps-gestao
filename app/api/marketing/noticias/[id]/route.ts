...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$executeRaw`
    ...
    WHERE id = ${id}
  `
  if ('dataInicio' in body) await prisma.$executeRaw`UPDATE "MarketingNoticia" SET "dataInicio" = ${body.dataInicio ? String(body.dataInicio) : null}::date WHERE id = ${id}`
  if ('dataFim' in body)    await prisma.$executeRaw`UPDATE "MarketingNoticia" SET "dataFim" = ${body.dataFim ? String(body.dataFim) : null}::date WHERE id = ${id}`
