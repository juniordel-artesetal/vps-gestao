...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (prioridade !== undefined) {
    await prisma.$executeRaw`
      UPDATE "SuporteChamado" SET "prioridade" = ${prioridade || 'normal'} WHERE "id" = ${id}
    `
  }
  if (responsavelNome !== undefined) {
    await prisma.$executeRaw`
      UPDATE "SuporteChamado" SET "responsavelNome" = ${responsavelNome || null} WHERE "id" = ${id}
    `
  }
  if (etiquetas !== undefined) {
    await prisma.$executeRaw`
      UPDATE "SuporteChamado" SET "etiquetas" = ${etiquetas || null} WHERE "id" = ${id}
    `
  }
