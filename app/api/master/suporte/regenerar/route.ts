...
const FAQ_SEED: { categoria: string; pergunta: string; resposta: string }[] = [
  ...
  { categoria: 'Produção', pergunta: 'Posso criar status de pedido personalizados?', resposta: 'Não. Os status são fixos: ABERTO, EM PRODUÇÃO, PRONTO, ENVIADO e CANCELADO. PRONTO é quando a produção terminou (aguardando expedir, se você tem setor de expedição); ENVIADO é quando a expedição foi concluída. O que você personaliza são os SETORES (Config → Produção) — as etapas por onde o pedido passa. O setor de expedição envia o pedido automaticamente ao ser concluído.' },
