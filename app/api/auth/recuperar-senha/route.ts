...
export async function POST(req: NextRequest) {
  try {
    ...
    const tokenId   = Math.random().toString(36).slice(2) + Date.now().toString(36)

    // Garante que a coluna existe (cria se não existir via upsert seguro)
    // Salva token na tabela User como campo extra — sem nova tabela
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#111827;color:#f9fafb;padding:32px;border-radius:16px;">
          <img src="${baseUrl}/logo.png" alt="VPS Gestão" style="height:40px;margin-bottom:24px;" />
          ...
          <p style="color:#6b7280;font-size:12px;margin:0;">
            Ou copie o link: <span style="color:#9ca3af;">${link}</span>
          </p>
        </div>
      `,
    })
