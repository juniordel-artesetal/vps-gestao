...
export async function GET(req: NextRequest) {
  try {
    ...
    const cf = await getCustosFixos(workspaceId)
    const custoFixoMes = cf.ativo ? Math.max(0, cf.custoFixoMensal || 0) : 0
    ...
    for (let i = 0; i < nMeses; i++) {
      ...
      mapaMes.set(`${ano}-${mes}`, { ano, mes, vendas: 0, taxas: 0, taxasCanal: 0, custoMateriais: 0, outrosCustos: 0, lucro: 0, custoFixo: 0, lucroReal: 0, itensVinculados: 0, itensTotal: 0 })
    }
    ...
    const meses = Array.from(mapaMes.values())
      ...
      .map(m => {
        const lucro = m.vendas - m.taxas - m.taxasCanal - m.custoMateriais - m.outrosCustos
        // Custo fixo do mês só conta quando houve venda (mês sem movimento não "gera prejuízo fixo" no gráfico).
        const custoFixo = m.vendas > 0 ? custoFixoMes : 0
        return { ...m, lucro, custoFixo, lucroReal: Math.round((lucro - custoFixo) * 100) / 100 }
      })
    ...
    const totais = meses.reduce((acc, m) => ({
      ...
      lucro: acc.lucro + m.lucro, custoFixo: acc.custoFixo + m.custoFixo, lucroReal: acc.lucroReal + m.lucroReal,
    }), { vendas: 0, taxas: 0, taxasCanal: 0, custoMateriais: 0, outrosCustos: 0, lucro: 0, custoFixo: 0, lucroReal: 0 })

    return NextResponse.json(serialize({
      estimado: true,
      meses,
      totais,
      usaCustoFixo: cf.ativo,
      usaCanais,
