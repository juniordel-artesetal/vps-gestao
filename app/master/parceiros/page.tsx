...
export default function MasterParceiros() {
  ...
  // Influenciadora: ativa (cortesia + aprova) ou encerra a cortesia, numa ação só.
  async function agirInflu(id: string, workspaceId: string, acao: 'ativar' | 'encerrar') {
    if (acao === 'encerrar' && !confirm('Encerrar a cortesia? Ela perde o acesso gratuito em 14 dias; a parceria continua (ela segue ganhando comissão).')) return
    setBusy(id)
    const r = await fetch(`/api/master/influenciadora/${workspaceId}/${acao}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({}) })
    const d = await r.json().catch(() => ({}))
    setBusy('')
    if (!r.ok) { alert(d.error || 'Erro'); return }
    carregar()
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto text-white">
      ...
      <div className="space-y-3 mb-8">
        {dados.pendentes.map(p => <PendenteCard key={p.id} p={p} busy={busy === p.id} agir={agir} agirInflu={agirInflu} />)}
      </div>
      ...
      <div className="space-y-2">
        {dados.aprovadas.map(p => (
          <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-lg p-3 text-sm flex flex-wrap items-center gap-x-4 gap-y-1">
            ...
            {p.selo === 'influenciadora' && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-orange-500/15 text-orange-400">✨ Influenciadora{p.cortesiaAtiva ? ' · cortesia ativa' : p.cortesiaEncerrada ? ' · cortesia encerrada' : ''}</span>}
            ...
            {p.selo === 'influenciadora' && p.workspaceId && p.cortesiaAtiva && (
              <button disabled={busy === p.id} onClick={() => agirInflu(p.id, p.workspaceId, 'encerrar')} className="text-xs bg-gray-800 hover:bg-red-800 border border-gray-700 rounded px-2.5 py-1 disabled:opacity-50">Encerrar cortesia</button>
            )}
            {p.selo === 'influenciadora' && p.workspaceId && !p.cortesiaAtiva && p.status === 'aprovada' && (
              <button disabled={busy === p.id} onClick={() => agirInflu(p.id, p.workspaceId, 'ativar')} className="text-xs bg-orange-600 hover:bg-orange-700 text-white rounded px-2.5 py-1 disabled:opacity-50">Ativar cortesia</button>
            )}
          </div>
        ))}
      </div>
...
function PendenteCard({ p, busy, agir, agirInflu }: { p: any; busy: boolean; agir: (id: string, a: 'aprovar' | 'recusar', c?: string) => void; agirInflu: (id: string, ws: string, a: 'ativar' | 'encerrar') => void }) {
  const [codigo, setCodigo] = useState(p.cupom || '')
  const completou = p.temSenha && p.temCodigo
  const ehInflu = p.selo === 'influenciadora' && p.workspaceId
  return (
    <div className={`bg-gray-900 border rounded-xl p-4 ${ehInflu ? 'border-orange-700/50' : 'border-amber-800/40'}`}>
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
        <span className="font-semibold">{p.nome}</span>
        {ehInflu && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-orange-500/15 text-orange-400">✨ Influenciadora</span>}
      ...
      <div className="flex flex-wrap items-center gap-2 mt-3">
        {ehInflu ? (
          // Influenciadora: cadastro já nasce com senha+cupom → ação única "Ativar influenciadora"
          // (liga a cortesia + aprova a parceira de uma vez).
          <>
            <span className="text-xs text-gray-400">cupom <b className="text-white">{p.cupom}</b></span>
            <button disabled={busy} onClick={() => agirInflu(p.id, p.workspaceId, 'ativar')} className="bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white rounded px-3 py-1 text-sm font-semibold">✨ Ativar influenciadora</button>
            ...
            <span className="text-[11px] text-gray-500">liga a cortesia + aprova a parceria</span>
          </>
        ) : (
          <>
