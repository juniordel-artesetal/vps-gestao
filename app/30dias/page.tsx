'use client'
// app/30dias/page.tsx
// Landing EVERGREEN do teste grátis de 30 dias do SOA (para Stories e parceiros divulgarem).
// Baseada na página da Mega Artesanal, MAS sem nada do evento (nome, datas, countdown, stands).
// Rota pública, sem auth, sem lead-gate (frictionless → clique direto no checkout). A Sofia não monta aqui.
import {
  Package, Calculator, Wallet, Users, ShoppingBag, KanbanSquare,
  Sparkles, Boxes, LineChart, FileSpreadsheet, Check,
} from 'lucide-react'

// ★ Link do checkout Hotmart do teste de 30 dias — troque AQUI se a oferta mudar. ★
...
function CtaHotmart({ className = '', bloco = false, texto = 'Começar meu teste grátis' }: { className?: string; bloco?: boolean; texto?: string }) {
  return (
    <a
      href={HOTMART_HREF}
      target="_blank"
...
// Depoimentos em imagem (prints reais). Para ADICIONAR novos: solte a imagem em /public e
// inclua o arquivo aqui; para depoimento em texto, use a lista DEPOIMENTOS_TEXTO abaixo.
const DEPOIMENTOS_IMG = ['/depoimento-1.jpeg', '/depoimento-2.jpeg', '/depoimento-3.jpeg']

// Depoimentos em texto (nome · cidade/nicho · texto). Cole os novos aqui.
const DEPOIMENTOS_TEXTO: { nome: string; contexto?: string; texto: string; foto?: string }[] = [
  // { nome: 'Fulana', contexto: 'Ateliê X · Belo Horizonte', texto: '...', foto: '/depoimento-x.jpeg' },
]
    ...
    <div className="min-h-screen bg-white text-gray-900" style={{ colorScheme: 'light' }}>
      ...
      <section className="px-5 pb-10">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mt-7 max-w-sm mx-auto"><CtaHotmart bloco className="py-4 text-base" texto="Quero testar 30 dias grátis" /></div>
        </div>
      </section>
      ...
      <footer className="px-5 py-8 text-center text-xs text-gray-400">
        ...
        <p className="mt-1">Teste grátis de 30 dias · sem compromisso</p>
      </footer>
    </div>
  )
}
