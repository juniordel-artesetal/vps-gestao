...
type Item = {
  ...
  slug?: string // deep link por produto
...
export default function LojaCliente() {
  ...
  useEffect(() => { setSelecao({}); setLinkCopiado(false) }, [detalhe])
  ...
  function fechar() {
    setDetalhe(null)
    if (typeof window !== 'undefined') { try { window.history.pushState({}, '', urlLoja()) } catch {} }
  }
    ...
    if (item.tipo === 'produto') {
      ...
      return (
        <div key={key} className="bg-white rounded-xl border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition group">
          <button onClick={() => abrir(item)} className="text-left">
          ...
          <div className="p-3 flex flex-col flex-1">
            ...
            <div className="mt-auto">
              <button onClick={() => abrir(item)} disabled={esgotadoTudo}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {detalhe && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4" onClick={() => fechar()}>
          <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl max-h-[92vh] flex flex-col rounded-t-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="relative">
              ...
              <button onClick={() => fechar()} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center shadow"><X size={16} /></button>
            </div>
            {galeria.length > 1 && (
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => { fechar(); setCarrinhoAberto(true) }} className="flex-1 py-3 rounded-xl text-white text-sm font-semibold" style={{ backgroundColor: cor }}>Ver carrinho</button>
                  </div>
                )
              })()}
            </div>
