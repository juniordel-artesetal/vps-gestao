...
export default function CalendarioPage() {
  ...
  const pedidosPorDia = pedidosFiltrados.reduce((acc, p) => {
    const d = p.dataEnvio
    if (!d) return acc
    if (['ENVIADO', 'PRONTO', 'CANCELADO'].includes(p.status)) return acc
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      {(() => {
        ...
        const atrasados = pedidosFiltrados.filter(p => p.dataEnvio && p.dataEnvio < hoje_key && p.status !== 'ENVIADO' && p.status !== 'PRONTO' && p.status !== 'CANCELADO')
        return (
