...
## Imagem própria do Combo na vitrine (David)
- `PrecCombo.imagem` (data URL, coluna separada via `ensureComboLoja`) — servida on-demand, NUNCA no SELECT da listagem (lista trocou `SELECT c.*` por colunas explícitas + `temImagem`).
- Upload no cadastro do combo (compressão client-side ≤700px) via PATCH `/api/precificacao/combos/[id]` (`imagem`/`removerImagem`). Preview admin: `/api/precificacao/combos/[id]/imagem`.
- Vitrine: `/api/loja/[slug]/combo-imagem/[comboId]` (público; combo ativo+visível). O card usa a imagem PRÓPRIA quando existe; senão, fallback pra imagem do 1º componente (`temImagemPropria` no `/api/loja/[slug]`).
