...
## Taxas de marketplace: ML por API oficial + monitoramento semanal (Canais)
- **Fonte única da taxa ML** = registro `mercadolivre` de `CatalogoCanal.regras`, lido por `resolverTaxaLocal`/`escolherRegra` (propaga p/ precificação, simulador, Resultado).
- **ML por API oficial** (`lib/mlTaxas.ts`): `consultarSaleFeeML(preco, listingTypeId)` → `GET api.mercadolibre.com/sites/MLB/listing_prices` (sale_fee_details: percentage_fee + fixed_fee). Cache `MLTaxaCache` (por listingType×preço amostra, `atualizadoEm`). `refreshMLTaxas()` reescreve as `regras` do ML no CatalogoCanal (variante gold_special=clássico / gold_pro=premium × faixas de preço). **Token só em env `ML_ACCESS_TOKEN`** (+ `ML_SITE_ID`). Sem token → fallback: mantém o catálogo/cache atual, não quebra. Cron `/api/cron/ml-taxas` (vercel.json `30 8 * * 1`).
- **Shopee/TikTok (sem API) — monitoramento** (`lib/canaisMonitor.ts`): tabelas `CanalMonitor` (canal, urlFonte, ultimaVerificacao, hashConteudo, ultimoStatus) e `CanalRevisao` (alerta pendente/resolvido). `verificarCanais()` baixa a página da fonte, compara hash com o último → se mudou, cria REVISÃO pendente (NÃO altera preço). Cron `/api/cron/canais-taxa` (vercel.json `0 8 * * 1`), gate `CANAIS_MONITOR_ATIVO=on` (senão só verifica). `?simular=shopee|tiktokshop|all` força mudança p/ teste; `?dryRun=1`.
- **Master**: `/api/master/canais-revisoes` (GET lista pendentes + monitores; POST `resolver`/`verificarAgora`). Card **"Taxas a revisar (N)"** no dashboard (`/api/master/dashboard?secao=stats` → `taxas_a_revisar`) + painel em `/master/canais` com "verificado em DD/MM" e botão "Verificar agora". Aprovação = Master revisa e edita o catálogo pelo PUT existente.
